...
export const TaskForm: React.FC<TaskFormProps> = ({ onAddTask }) => {
  ...
  return (
    <form
      ...
      className="w-full max-w-full bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 focus-within:border-indigo-500/70 focus-within:ring-2 focus-within:ring-indigo-500/20 rounded-2xl p-2.5 sm:p-3 shadow-md dark:shadow-xl transition-all box-border"
    >
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2.5 sm:gap-3 w-full max-w-full">
        {/* Left Input Row */}
        <div className="flex items-center gap-2 flex-1 min-w-0 w-full">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white shrink-0 shadow-md shadow-purple-500/30 hidden sm:flex">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs shrink-0 w-full lg:w-auto justify-start sm:justify-end">
          ...
          <div className="relative flex-1 sm:flex-initial min-w-[110px]">
            <select
              ...
              className="w-full appearance-none bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700/70 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700/60 rounded-xl px-2.5 py-2 pr-7 text-xs focus:outline-none focus:border-indigo-500 cursor-pointer transition-colors font-semibold min-h-[36px]"
            >
          ...
          <button
            ...
            className={`flex items-center justify-center border rounded-xl px-3 py-2 transition-all min-h-[36px] font-bold cursor-pointer shrink-0 ${priorityStyles}`}
          ...
          <button
            type="submit"
            disabled={!title.trim()}
            className="w-full sm:w-auto bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-xs px-5 py-2 rounded-xl transition-all shadow-md shadow-purple-500/25 flex items-center justify-center gap-1.5 shrink-0 min-h-[36px] active:scale-95 cursor-pointer"
          >
